    ...
    def capture(cls, image, result, max_width, top):
        return cls(
            ...
            mask=np.ascontiguousarray(
                result.vision.segmentation.mask, dtype=np.bool_),
            depth=np.ascontiguousarray(result.depth.depth, dtype=np.float32),
            boxes=np.ascontiguousarray(
                result.vision.detection.boxes, dtype=np.float32),
            scores=np.ascontiguousarray(
                result.vision.detection.scores, dtype=np.float32),
            graspgroup=np.ascontiguousarray(
                result.grasp.graspgroup, dtype=np.float64),
            camera_k=np.ascontiguousarray(result.camera_K, dtype=np.float64),
...
class SnapshotCache:
    ...
    def put(self, snapshot):
        if snapshot.nbytes > self.max_bytes:
            raise ValueError("frame exceeds output snapshot cache capacity")
